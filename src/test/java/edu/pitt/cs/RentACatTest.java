// to get to html: target/site/jacoco/index.html or something like that (see readme for actual path)
...
public class RentACatTest {
	...
	public void testGetCatNullNumCats0() {
		assertNull("getCat(2) returns a cat when it should return null", r.getCat(2));
	}
	...
	public void testGetCatNumCats3() {
		...
		r.addCat(c3);
		//Mockito.when(c2.getId()).thenReturn(2);
		assertNotNull("getCat(2) returns null when there are 3 cats", r.getCat(2));
		assertEquals("getCat(2) does not return id of 2", 2, (r.getCat(2)).getId());
	}
	...
	public void testCatAvailableFalseNumCats0() {
		assertFalse("getCat(2) returns true when the cat is avalable", r.catAvailable(2));
	}
	...
	public void testCatAvailableTrueNumCats3() {
		...
		//Mockito.when(c2.getRented()).thenReturn(false);
		assertTrue("getCat(2) returns false when the cat is not avalable", r.catAvailable(2));
	}
	...
	public void testCatExistsFalseNumCats0() {
		assertFalse("getCat(2) returns true when it exists", r.catExists(2));
	}
	...
	public void testCatExistsTrueNumCats3() {
		...
		r.addCat(c2);
		r.addCat(c3);
		assertTrue("getCat(2) returns false when it does not exist", r.catExists(2));
	}
	...
	public void testListCatsNumCats0() {
		// DONE - Bekah
		assertEquals("Listing 0 cats is incorrect", "", r.listCats());
	}
	...
	public void testListCatsNumCats3() {
		...
		//Mockito.when(c2.getRented()).thenReturn(false);
		//Mockito.when(c3.getRented()).thenReturn(false);
		assertEquals("Listing three cats is incorrect", "ID 1. Jennyanydots\nID 2. Old Deuteronomy\nID 3. Mistoffelees\n", r.listCats());
	}
	...
	public void testRentCatFailureNumCats0() {
		assertFalse("rentCat(2) returns false when the cat either doesn't exist or is already rented", r.rentCat(2));
	}
	...
	public void testRentCatFailureNumCats3() {
		...
		r.addCat(c3);
		Mockito.when(c2.getRented()).thenReturn(true);
		assertFalse("rentCat(2) returns false when the cat is already rented", r.rentCat(2));
	...
	public void testReturnCatFailureNumCats0() {
		// DONE - Bekah
		assertFalse("returnCat(2) returns true when there are no cats to return", r.returnCat(2));
	}
	...
	public void testReturnCatNumCats3() {
		...
		assertTrue("returnCat(2) returns false when there is a cat to return", r.returnCat(2));
